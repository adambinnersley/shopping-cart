...
class Order extends Basket{
    ...
    public function fetchOrders($status = 2, $userID = 0, $start = 0, $limit = 50, array $additional = []) {
        ...
        return $this->db->query("SELECT * FROM `{$this->config->table_basket}` as `bskt`, `{$this->config->table_users}` AS `usr` WHERE `bskt`.`customer_id` = `usr`.`id`".SQLBuilder::createAdditionalString($additional)." ORDER BY `date` DESC".($limit >= 1 ? " LIMIT ".intval($start).", ".intval($limit) : '').";", array_values($additional));
    }
    ...
    public function listOrders($status = 2, $start = 0, $limit = 50, array $additional = [], $count = false) {
        return $this->db->query("SELECT ".($count === false ? "*" : "count(`{$this->config->table_basket}`.`order_id`) as `count`")." FROM `{$this->config->table_basket}`, `{$this->config->table_users}` WHERE `{$this->config->table_basket}`.`customer_id` = `{$this->config->table_users}`.`id` AND `{$this->config->table_basket}`.`customer_id` != '0'".(is_numeric($status) ? " AND `{$this->config->table_basket}`.`status` = ".intval($status) : "").SQLBuilder::createAdditionalString($additional)." ORDER BY `date` DESC".($limit >= 1 ? " LIMIT ".intval($start).", ".intval($limit) : '').";", (!empty($additional) ? array_values($additional) : []));
    }
