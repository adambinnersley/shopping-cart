...
class Basket{
    ...
    public function getBasket($orderNo = '', $additional = []){
        if(empty(trim($orderNo))){$where = ['status' => 1, 'customer_id' => ($this->user_id === 0 ? 'IS NULL' : $this->user_id), 'sessionid' => session_id()];}
        else{$where = ['order_no' => ['LIKE', '%'.$orderNo.'%']];}
    ...
    protected function getProducts($orderID = '', $additional = []){
        if(empty(trim($orderID)) || !is_numeric($orderID)){$orderID = $this->getBasket('', $additional)['order_id'];}
    ...
    public function addItemToBasket($product_id, $quantity = 1, $update = true, $additional = []) {
        ...
        if(is_numeric($product_id) && is_numeric($quantity) && is_array($this->product->getProductByID($product_id))){
