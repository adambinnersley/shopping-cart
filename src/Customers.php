    ...
    protected function addRequest($uid, $email, $type, $sendmail) {
        ...
        if($return['error'] === false && $sendmail === true) {
            ...
            elseif($type == "reset"){
                ...
                $mailsent = Mailer::sendEmail($email, $subject, sprintf($this->config->email_reset_altbody, $this->config->site_url, $this->reset_page, $this->key),  Mailer::htmlWrapper($this->config, sprintf($this->config->email_reset_body, $this->config->site_url, $this->reset_page, $this->key), $subject), $this->config->email_from_address, $this->config->email_from_name);
            }
            if($mailsent !== true) {
