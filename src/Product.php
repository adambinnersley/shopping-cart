...
class Product extends Category{
    ...
    protected function addImage($image) {
        ...
        if(is_array($image) && $this->imageUpload->uploadImage($image)) {
            $additionalInfo['image'] = $image['name'];
    ...
    protected function getProduct($where, $active = true) {
        if(is_array($where)) {
            ...
            if(is_array($productInfo)) {
                $productInfo['categories'] = $this->listProductCategories($productInfo['product_id']);
    ...
    /**
     * Build an array containing categories and products with links to build navigation from array
     * @param string $currentURL The current URL that the user is on
        ...
        if(is_array($categories)){
            ...
            foreach($categories as $c => $category) {
                $nav[$c] = [
                    ...
                    'link_order' => ($c + 1),
                    'children' => $this->buildProductNavArray($this->config->store_url.$category['url'].'/', $category['id'])
                ];
            }
            return $nav;
        }
    ...
    /**
     * Build an array for the navigation array containing the products
     * @param string $categoryURL This should be the URL of the category
